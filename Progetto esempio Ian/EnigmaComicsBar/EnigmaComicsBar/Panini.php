...
<html>
    <head>
        ...
        <title>Panini - Enigma Comics Bar</title>
    </head>
        ...
        <div id="wrapper">
            ...
            <div id="Panini" class="section">
                ...
                <hr>
		          ...
		          if(isset($_GET['del'])){
		            $id=$_GET['del'];
								$temp=$access->getPanino($id);
								$image=$temp['Immagine'];
		            if(is_numeric($id) && $id>=0){//controllo che l'id sia un numero positivo
		              if($access->EliminaPanino($id)){
											unlink("images/Panini/$image");
		                  ?>
		                  <div class="messaggio">Rimozione del panino "<span xml:lang="en" lang="en"><?php echo $temp['Nome']; ?></span>" effettuata.</div>
		                  <?php
		              }
		            }
		          }
								...
								if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
									...
									if(!$error){
											if(!move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $targetFile)) {
												...
												if($access->saveNewPanino(mysqli_real_escape_string($access->connessione,$nome), $imageName, $prezzo, mysqli_real_escape_string($access->connessione,$descrizione))){
													$messaggio .= "<li><span xml:lang=\"en\">Panini</span> inserito.</li>";
												}
											}
									}else{
	          ...
	          $result=$access->getPanini();
	        if(count($result)>0){?>
	          <?php foreach($result as $Panini){?>
	            ...
	            <div class="descrizione"> Ingredienti: <?php echo $Panini['Ingredienti'];?> </div>
                <div class="prezzo">€<?php echo $Panini['Prezzo'];?> </div>
	            <?php
	            if($admin){?>
	            <div id="Modifiche">
	              <div id="Modifica"><a href="Modifica_Panino.php?mod=<?php echo $Panini['ID'];?>">Modifica</a></div>
	              <div id="Elimina"><a href="Panini.php?del=<?php echo $Panini['ID'];?>" onclick="return(confirm('stai eliminando il drink: <?php echo $Panini['Nome'];?>'))">Elimina</a></div>
	            </div>
	            <?php
	          }?>
	           </div>
	        ...
	        if($admin){
						?>
	          <div class="Inserimento"> Inserisci un nuovo panino: </div>
	          <form class="formInserimento" action= "<?php echo $_SERVER [ 'PHP_SELF']; ?>"  method="post" enctype="multipart/form-data">
							...
							<p><label for="descrizione"></label></p>Ingredienti: <p><textarea id="descrizione" name="descrizione" rows="10" cols="50" ><?php echo $descrizione;?></textarea></p>
