...
<html>
    <head>
        ...
        <title>Modifica panino - Enigma Comics Bar</title>
    </head>
    ...
    <body>
        ...
        <div id="wrapper">
            ...
            <div id="modPanino" class="section">
            	<h1>Modifica panino</h1>
            	<hr>
            <?php
		          if($admin){ ?>
	            ...
	            if(isset($_GET['mod'])){
	              $id=$_GET['mod'];
	              if(is_numeric($id)){
	              $result=$access->getPanino($id);
	              if($result){
	                $messaggio="<ul class=\"messaggio\">";
	                  if (isset($_POST["submit"])){
	                    $error=0;
	                    $nome=$_POST["nome"];
	                    $targetDir = "images/Panini/";//percorso della cartella dove inserire il file
	                    ...
	                    if($result['Nome']!=$nome){
	                      if(strlen($nome)<3){
	                        ...
	                        $error=1;
	                      }elseif(!$access->controllaNomePanino(mysqli_real_escape_string($access->connessione,$nome))){
	                    ...
	                    if(!$error){// se non ci sono stati errori carico l'immagine se c'è da inserirla e salvo il Panino
	                        if($result['Immagine']!=$imageName && !move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $targetFile)) {
	                          ...
	                          if($access->UpdatePanino($id, mysqli_real_escape_string($access->connessione,$nome), $imageName, $prezzo,mysqli_real_escape_string($access->connessione,$descrizione))){
															if($result['Immagine']!=$imageName){
																$image=$result['Immagine'];
																unlink("images/Panini/$image");
															}
	                            $messaggio .= "<li>Panino modificato.</li>";
	                          }
	                        }
	                    }else{
	                ...
	                $result=$access->getPanino($id);// riaggiorno il panino
	                ?>
	                  <div class="box"><div class="name">
	                    <span xml:lang="en" lang="en"><?php echo $result['Nome']; ?></span>
	                  </div>
	                      <div class="boxImage"><img class="image" alt="Foto di un panino" src="images/Panini/<?php echo $result['Immagine']; ?>">
	                </div>
	                <div class="descrizione">Ingredienti: <?php echo $result['Ingredienti'];?> </div>
	                ...
	                if(isset($_POST["submit"])){
	                  ...
	                  <div class="Inserimento"> Modifica panino: </div>
	                  <form class="formInserimento" action="Modifica_Panino.php?mod=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
	                    ...
	                    <p><label for="descrizione">Ingredienti: </label></p> <textarea id="descrizione" name="descrizione" rows="10" cols="50" ><?php echo $result['Ingredienti'];?></textarea>
	                  ...
	                  <?php
							...
							<div id="errorMsg">Non hai selezionato nessun panino, <a href="Panini.php">selezionane uno</a></div>
							</div>
						<?php } 

	            }else{?>
	          	<div class="AreaRiservata"><p>Area riservata.</p> <p><a href="Home.php">Torna alla <span xml:lang="en" lang="en">home</span></a></p></div>
							</div>
						<?php }
